...
export class CategoryService implements CommonDAO<Category>{
  ...
  search(search: SearchObject): Observable<Category[]> {
    return this.http.post<Category[]>(this.baseUrl + '/search', search);
  }
  getStat(search: SearchObject): Observable<Category[]> {
    return this.http.post<Category[]>(this.baseUrl + '/stats', search);
  }
}
